...
def report(agg: dict) -> str:
    L = []
    L.append(
        f"文件 {agg['files']}（读失败 {agg['files_unreadable']}）｜行 {agg['lines']:,}"
    ...
    L.append(
        f"  买侧: p10={pct(bh, 0.10):.0f} p50={pct(bh, 0.50):.0f} p90={pct(bh, 0.90):.0f} "
        f"p99={pct(bh, 0.99):.0f} max={max(bh) if bh else 0}"
    )
    L.append(
        f"  卖侧: p10={pct(ah, 0.10):.0f} p50={pct(ah, 0.50):.0f} p90={pct(ah, 0.90):.0f} "
        f"p99={pct(ah, 0.99):.0f} max={max(ah) if ah else 0}"
    )
    top = sorted(bh.items(), key=lambda kv: -kv[1])[:6]
    L.append(f"  买侧最常见档数: {top}")
    tot = sum(bh.values()) or 1
    L.append(
        f"  档数 ≥50 的帧占比: {sum(v for k, v in bh.items() if k >= 50) / tot * 100:.1f}%"
        f"｜≥90: {sum(v for k, v in bh.items() if k >= 90) / tot * 100:.1f}%"
    )
    L.append("")
    L.append("覆盖边界")
    L.append(
        f"  单侧空盘口: 买 {agg['bid_side_empty']:,} 帧 / 卖 {agg['ask_side_empty']:,} 帧"
    )
    L.append(
        f"  买侧含 0.01: {agg['bid_has_001']:,} 帧｜卖侧含 0.99: {agg['ask_has_099']:,} 帧"
    )
    ...
    L.append(
        f"  非 0.01 步长帧: 买 {agg['nonstd_step_bid']:,} / 卖 {agg['nonstd_step_ask']:,}"
    ...
    L.append(
        f"  {'>60':>12}s : {iv.get('>60', 0):>12,} ({iv.get('>60', 0) / tot_iv * 100:5.1f}%)"
    )
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="审计原始 L2 包的深度完整性（只读全量扫描）"
    )
    ap.add_argument(
        "--root", default=DEFAULT_ROOT, help="原始数据根目录（默认 ../data）"
    )
    ...
    dates = args.date or sorted(
        os.path.basename(p)
        for p in glob.glob(os.path.join(args.root, "2026-*"))
        if os.path.isdir(p)
    )
    files = []
    for d in dates:
    ...
    if not args.quiet:
        print(
            f"扫描 {len(files)} 个文件，日期 {dates}，并发 {args.jobs}…",
            file=sys.stderr,
        )
